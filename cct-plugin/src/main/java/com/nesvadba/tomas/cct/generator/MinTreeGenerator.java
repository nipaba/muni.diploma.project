...
/**
 * Generator minimoveho stromu
 * 
 * @author Nesvadba Tomáš, učo 395902 - Diplomová práce
 *
 */
public class MinTreeGenerator extends CCTGenerator {
    ...
    public CCT createCCT(Point[][] imgpoints) {
        ...
        proccess(points[0][0], 256);

        // Finished
        StringBuilder str = new StringBuilder();
        str.append("MinTreeGenerator - Building");
    ...
    protected int flood(int level) {
        ...
        while (m <= 255 && (quegeMap.get(m) == null || quegeMap.get(m).isEmpty())) {
            m++;
        }
        if (m <= 255) {
